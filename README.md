...
If your device drivers do not support any of these extensions, the program won't launch. In the case of `VK_KHR_UNIFIED_IMAGE_LAYOUTS_EXTENSION`, you can try disabling it in the `init.cpp/init_vulkan()` function. You can check the supported extensions of your device with the `vulkaninfo` command. E.g.: `vulkaninfo | grep -i VK_KHR_RAY_TRACING_PIPELINE` will check whether you can use the Vulkan rt pipeline extension in your GPU and driver.
...
- **Lights manager and other controls with imgui:** Runtime addition/removal/modification of point lights and directional lights (I have limited them to 10 but the limit can be changed in compile time). Other controls: Background color picker, environment map selection, random sampling toggle (recommended to leave this on, otherwise you end up with a biased Monte-Carlo integration), rt recursion depth, number of bounces (samples) after each intersection, object scale and rotation.
...
- [Understandable RayTracing in 256 lines of bare C++](https://github.com/ssloy/tinyraytracer) - Before proceeding with the Vulkan RT pipeline, I convinced myself that I truly understand ray tracing by trying to reproduce in CPU the results from that project.
...
- **Denoising:** Critical for image quality. Maybe https://github.com/RenderKit/oidn is a good starting point.
- **More efficient algorithm:** I consider to implement a ReSTIR-like algorithm with intelligent caching in the future. I have to study these [notes](https://intro-to-restir.cwyman.org/presentations/2023ReSTIR_Course_Notes.pdf) before that.
