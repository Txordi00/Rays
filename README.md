...
- [ ] **Mipmapping:** To avoid some artifacts when sampling textures from a distance (Moiré pattern). I implemented it at the initalization level already, in the sense that mip levels are created and properly reach the shaders. But the standard mipmapping technique integrated into the Vulkan graphics pipeline does not work in ray tracing. I need to manually compute the derivatives of the pixel space -> texel space map manually and then fetch the texture with `textureGrad()`. Maybe take it from [this article](https://research.nvidia.com/publication/2021-04_improved-shader-and-texture-level-detail-using-ray-cones)?
